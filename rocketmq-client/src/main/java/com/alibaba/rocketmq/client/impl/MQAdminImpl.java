...
public class MQAdminImpl {
    ...
    public long earliestMsgStoreTime(MessageQueue mq) throws MQClientException {
        ...
        {
        	/**
        	 * 订阅主题的路由信息有空，需要从nameserver上更新路由信息
        	 */
            this.mQClientFactory.updateTopicRouteInfoFromNameServer(mq.getTopic());
            /**
             * 再从新获取一边路由的broker信息
        ...
        /**
         * 如果这次获取到broker信息了，则需要获取最早消息存储时间
         */
        if (brokerAddr != null) {
            try {
                return this.mQClientFactory.getMQClientAPIImpl().getEarliestMsgStoretime(brokerAddr, //broker地址 
                		mq.getTopic(), //订阅的主题
                		mq.getQueueId(), //队列编号
                        timeoutMillis); //超时时间
            } catch (Exception e) {
