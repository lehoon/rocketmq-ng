...
public class MQClientAPIImpl {
    ...
    /**
     * @描述: 查询broker上指定订阅主题消息队列的消息存储的开始时间 
     * @作者: zgzhang@txbds.com
     * @日期:2016年11月23日
     * @修改内容
     * @参数：@param addr     broker地址---ip:port
     * @参数：@param topic    订阅主题
     * @参数：@param queueId  消息队列编号
     * @参数：@param timeoutMillis  超时时间
     * @参数：@throws RemotingException
     * @参数：@throws MQBrokerException
     * @参数：@throws InterruptedException     
     * @return long   时间   
            ...
            throws RemotingException, MQBrokerException, InterruptedException 
    {
        ...
        RemotingCommand response = this.remotingClient.invokeSync(
        		//获取broker的vip通道地址, ip:(listenport - 2)
        		MixAll.brokerVIPChannel(this.clientConfig.isVipChannelEnabled(), //是否启用vip通道 
        				addr), //broker地址
                request, //请求报文
                timeoutMillis); //超时时间
    ...
    public void sendHearbeat(//
                             ...
                             final HeartbeatData heartbeatData, // 心跳包数据
